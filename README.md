# Support

Help and contact information for the Affirmation app.

## Contact

If you have questions, issues, or feedback, feel free to reach out:

**Email:** [nordgrenstudio@gmail.com](mailto:nordgrenstudio@gmail.com)
We usually respond within 24-48 hours.

---

## Frequently Asked Questions

### How do I customize the app?
Open Settings to choose your vibes, adjust the card style, change themes, and update the refresh frequency for the app and widget.

### How do I add the widget?
Long-press your Home Screen, tap the add button, search for **Affirmation**, and choose the widget size you want.

### Where is my data stored?
Your settings, saved affirmations, and custom affirmations are stored locally on your device.

### I purchased premium, but it is not unlocked
Open the paywall or settings area and use **Restore Purchases**. If it still does not work, contact support.

### How do I cancel my subscription?
Subscriptions are managed by Apple. Open the App Store, tap your profile, then go to **Subscriptions**.

---

## Report a Bug

If something is not working as expected, please include:
- What happened
- Steps to reproduce
- Your device model and iOS version

---

## Feature Requests

Have an idea to improve Affirmation? Send it to us. User feedback helps shape future updates.

---

## Privacy & Terms

- [Privacy Policy](https://anordgren.github.io/affirmation-privacy/)
- [Terms of Use](https://www.apple.com/legal/internet-services/itunes/dev/stdeula/)

---

## About

Affirmation is designed to bring curated affirmations, custom vibes, and widget-friendly calm to your daily routine.
